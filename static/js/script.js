...
async function selectStoreForCustomer(store) {
  ...
  customerElements.selectedStoreInfo.innerHTML = `
    ...
    <div class="flex items-center space-x-2">
      <a href="${naverMapUrl}" target="_blank" title="네이버 지도로 보기" class="hover:opacity-75" rel="noopener noreferrer">
        <img
          src="static/js/naver_icon.webp"
          onerror="this.onerror=null; this.src='static/js/naver_icon.png';"
          alt="네이버 지도"
          class="w-6 h-6"
        />
      </a>
      <a href="${kakaoMapUrl}" target="_blank" title="카카오맵으로 보기" class="hover:opacity-75" rel="noopener noreferrer">
        <img
          src="static/js/kakao_icon.png"
          alt="카카오맵"
          class="w-6 h-6"
        />
      </a>
...
function initStorePage() {
  ...
  // ✅ KST '오늘'을 자동 선택하여 오늘 예약을 즉시 표시
  const today = todayKSTYMD();
  selectStoreDate(today);
}
